...
**Never print, echo, or cat a token value** - output lands in the conversation transcript. Check for presence by name only; move values with assignments, never through displayed output.
...
If set, you're ready. Skip to Step 2.

### B) Token is in a `.env` file under `VERCEL_TOKEN`

```bash
grep -q '^VERCEL_TOKEN=' .env 2>/dev/null && echo "found VERCEL_TOKEN in .env"
```

If found, export it (the assignment never displays the value):
...
Look for variable **names** that suggest a Vercel token - list names only, never values:

```bash
grep -io '^[a-z_]*\(vercel\|vc\)[a-z_]*=' .env 2>/dev/null
```

Pick the likely variable from the names, then export it as `VERCEL_TOKEN`:
...
- **Never print, echo, or cat a token value.** Transcript output persists and leaves the machine. Test presence with `[ -n "$VAR" ]` or `grep -q`; move values only via assignments.
